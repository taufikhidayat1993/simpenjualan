	...
	error_reporting(0);
		if(isset($_POST['queryString'])) {
			$queryString = $_POST['queryString'];
			  ...
			  $sql="
SELECT ITEM_CODE,ITEM_NAME,QTY_STOCK FROM RS_MASTER_ITEM WHERE ITEM_NAME LIKE '%$queryString%' ORDER BY ITEM_NAME ";
...
$row_count = sqlsrv_num_rows( $stmt );
  ...
  <div class="pane-hScroll2">
				...
				<th style="width:280px;">NAMA OBAT</th><th style=width:250px;>KD</th></tr></thead></table>
				<div class="pane-vScroll2"> <table  width="100%" class="table table-hover">';
					if ($row_count > 0){
                                    while($data=sqlsrv_fetch_array($stmt,SQLSRV_FETCH_ASSOC)){
									$no_rm=$data['ITEM_CODE'];
	         			echo '<tbody><tr  class="rows" onClick="filobat(\''.addslashes($data['ITEM_NAME']).'\');">
						<td style="width:280px;" >'.$data['ITEM_NAME'].'</td><td >'.$data['ITEM_CODE'].'</td></tr>';
	         		}
